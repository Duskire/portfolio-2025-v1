...
import { CarouselSet } from "./carousels";
...
export default function ProjectCarousel({
  ...
  return (
    <div id={`${carousel.id}-carousel`} className="relative w-full flex justify-center" data-carousel="static">
      {/* Carousel wrapper */}
      <div className={`relative overflow-hidden rounded-lg ${imageHeight}`}>
          ...
          >
            <div className="flex justify-center items-center w-full h-full">
              <img
                src={item.image}
                alt={item.alt}
                className={`${imageWidth} ${imageHeight} object-cover rounded-lg`}
              />
            </div>
          </div>
        ))}
